...
<?php
$hero_title = bpm_meta( 'page_hero_title', 'Аренда и эксплуатация кранов с экипажем в Беларуси' );
$hero_text  = bpm_meta( 'page_hero_text', 'Грузоподъёмная строительная техника: башенные, автомобильные и гусеничные краны. Полный комплекс услуг — аренда, монтаж, обслуживание и проектные работы. Работаем с 2007 года на всей территории РБ.' );
$hero_bg    = bpm_meta( 'page_hero_bg', get_template_directory_uri() . '/img/hero-bg.webp' );
?>
<!-- Hero -->
...
<?php
$faq_defaults = array(
    array( 'Работаете ли вы по всей Беларуси?', 'Да, группа компаний БПМ Альянс работает с 2007 года на всей территории Республики Беларусь. Базируемся в Минске, но обеспечиваем выезд техники на объекты по всей стране.' ),
    array( 'Техника сдаётся с экипажем?', 'Да, техника предоставляется в аренду с экипажем и, при необходимости, со стропальщиками. Все специалисты имеют необходимые допуски и квалификацию.' ),
    array( 'Есть ли лицензии и допуски?', 'Все предприятия группы компаний имеют требуемые лицензии и разрешения. Работы выполняются строго по нормативам и утверждённым проектам производства работ.' ),
    array( 'Кто отвечает за безопасность работ?', 'Безопасность обеспечивается штатом квалифицированных специалистов, современной технической базой и необходимым оборудованием. Все работы ведутся по утверждённым ППРк и под контролем ответственных лиц.' ),
);
$faq_items = array();
for ( $i = 1; $i <= 6; $i++ ) {
    ...
    <div class="faq-list">
      ...
      <div class="faq-item">
        ...
        <div class="faq-item__answer">
          <div class="faq-item__answer-inner"><?php echo esc_html( $faq[1] ); ?></div>
        </div>
      </div>
      <?php endforeach; ?>
    </div>
